...
// Normalizes free-form tags (BR-3-10): trim, lowercase, collapse whitespace to dashes, strip any
// character outside [a-z0-9-], drop empties/over-long, dedupe, and cap the count. Matches the DB
// CHECK constraint on question_tags.tag ('^[a-z0-9-]{1,50}$').
public static partial class QuestionTagNormalizer
{
    public const int MaxTagLength = 50;
    public const int MaxTagsPerQuestion = 10;
    ...
    public static List<string> Normalize(IEnumerable<string>? rawTags)
    {
        ...
        foreach (var raw in rawTags)
        {
            ...
            result.Add(slug);
            if (result.Count >= MaxTagsPerQuestion)
                break;
        }
